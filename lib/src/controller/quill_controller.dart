...
class QuillController extends ChangeNotifier {
  ...
  void replaceText(
    ...
    bool removeEmbedProgramatically = false,
    @experimental bool shouldNotifyListeners = true,
  }) {
    assert(data is String || data is Embeddable || data is Delta);

    if (!removeEmbedProgramatically) {
      if (len > 0 && (data == null || (data is String && data.isEmpty))) {
        // 1) if deleting a single char immediately _after_ an embed
        if (index > 0) {
          final before = document.querySegmentLeafNode(index - 1).leaf;
          if (before is Embeddable) {
            updateSelection(
              TextSelection.collapsed(offset: before!.documentOffset),
              ChangeSource.local,
            );
            return;
          }
        }
        // 2) if a multi‐char deletion would span any embed
        final slice = document.toDelta().slice(index, index + len);
        final spansEmbed =
            slice.toList().any((op) => op.isInsert && op.data is Map);
        if (spansEmbed) {
    ...
    // ─── EMBED‑SAFE BACKSPACE / DELETE ─────────────────────────────────────

    Delta? delta;
    if (len > 0 || data is! String || (data.isNotEmpty)) {
      delta = document.replace(index, len, data);
      // re‑apply any inline toggledStyle you were carrying
      final inline = Style.attr(Map.fromEntries(
        toggledStyle.attributes.entries
            .where((e) => e.value.scope != AttributeScope.block),
      ));
      var retainNeeded = inline.isNotEmpty &&
          delta.isNotEmpty &&
          delta.length <= 2 &&
          delta.last.isInsert;
      if (retainNeeded &&
          delta.length == 2 &&
          delta.last.data == '\n' &&
          !inline.values.any((a) => !a.isInline)) {
        retainNeeded = false;
      }
      if (retainNeeded) {
        final r = Delta()
        ...
        document.compose(r, ChangeSource.local);
      }
    }

    // adjust the selection/cursor if requested
    if (textSelection != null) {
      if (delta == null || delta.isEmpty) {
        ...
        final user = Delta()
          ...
          ..insert(data)
          ..delete(len);
        final shift = getPositionDelta(user, delta);
        _updateSelection(
    ...
    if (ignoreFocus) ignoreFocusOnTextChange = true;
  ...
  void handleDelete(
      int cursorPosition, bool forward, bool removeEmbedProgramatically) {
    // compute the actual offset we’re about to delete
    final offset = forward ? cursorPosition : cursorPosition - 1;
    if (offset >= 0 && offset < document.length) {
      final leaf = document.querySegmentLeafNode(offset).leaf;
      if (leaf is Embeddable) {
        // move the cursor to just before that embed
        updateSelection(
          TextSelection.collapsed(offset: leaf!.documentOffset),
          ChangeSource.local,
        );
        return; // swallow the delete
      }
    }
    // otherwise fall back to any custom handler
